...
export default function App() {
  ...
  const handleGenerateUml = useCallback(async () => {
    setUmlError('');
    setUmlLoading(true);
    // --- YENİ KOD: Backend Bağlantı Fonksiyonları ---

    // 1. Sohbet mesajlarını veritabanına kaydeden fonksiyon
    const saveChatToBackend = async (text) => {
      if (!text.trim()) return; // Boş mesajları gönderme

      try {
        // Laravel API endpoint'ine POST isteği atıyoruz
        // Proje ID'si şimdilik statik olarak 1 verildi
        const response = await fetch('http://127.0.0.1:8000/api/projects/1/chat-logs', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            message: text,
            user_name: "Zid Alkhani", // Kullanıcı adı
            sent_at: new Date().toISOString() // Şu anki zaman
          }),
        });

        if (response.ok) {
          console.log("Mesaj başarıyla veritabanına kaydedildi.");
        }
      } catch (error) {
    ...
    // 2. Analiz butonuna basıldığında hem veriyi kaydeden hem de analizi başlatan yeni fonksiyon
    const handleGenerateAndSave = async () => {
      // Önce metni veritabanına kaydet
      await saveChatToBackend(meetingText);

      // Sonra orijinal analiz fonksiyonunu çalıştır
      handleGenerateUml();
    };
